...
class ConsoleGame:
    ...
    def render_answer(self):
        ...
        for i, row in enumerate(_map):
            order = str(i).rjust(2, ' ')
            map_text = ''.join(row)
            row_neighbour_text = ' '.join(self.count_neighbour_vector(row, FILLED_CHAR, is_output_str=True))
            line = f"{order}|{map_text}|{row_neighbour_text}"
            map_lines.append(line)

        column_counts = self.count_neighbours(_map.T, count_value=FILLED_CHAR, is_output_str=True)
        max_count_length = max([len(neighbour_counts) for neighbour_counts in column_counts])
        column_counts = [column + [' '] * (max_count_length-len(column)) for column in column_counts]
