...
#%%
def AIcontrol(step, game, countP, countL,Level,setLevel):
    if Level <= setLevel:
        result = False
        x,y = step.probs
        if x>=0:
            result, step.Map = game.paint(x,y)
            print(f'paint at {step.probs}\tresult: {result} countP: {countP} countL: {countL} Level:{Level}-{setLevel}')
        game.render()
        #time.sleep(0.1)
        ...
        if result:
            if AIcontrol(step.nextMap,game, countP+1, countL,Level+1,setLevel):
                return True
            else:
                game.erase(x,y)
                if step != None:
                    if countP==0 and x<7-countP:
                        if AIcontrol(step.nextMap,game, countP, countL,Level+1,setLevel):
                            return True
                        else:
                            game.erase(x,y)
                            return False
            ...
            if game.row_counts[y]==[]:
                ...
                if countP>0:
                    ...
                    if AIcontrol(step.nextMap,game, countP, countL,Level+1,setLevel):
                        ...
                        game.erase(x,y)
                        if x<0:
                            AIcontrol(stepPlay((-1,0)),game, 0, 0, 0,setLevel+1)
                        return False
            else:
                if countP > 0:
                    if countL < len(game.row_counts[y])-1:
                        if AIcontrol(step.nextMap,game, 0, countL+1,Level+1,setLevel):
                            return True
                        else:
                            game.erase(x,y)
                            return False
